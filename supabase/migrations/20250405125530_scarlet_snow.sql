/*
  ...
  1. Changes
    - Make all table creation idempotent with IF NOT EXISTS checks
    - Safely drop and recreate policies and functions
    - Ensure all operations can be reapplied without errors
    - Fix any conflicting migrations for the quote competition system
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'quote_themes') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'quote_entries') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'quote_votes') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'quote_winners') THEN
    CREATE TABLE quote_winners (
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
      entry_id uuid REFERENCES quote_entries(id) ON DELETE CASCADE,
      theme_id uuid REFERENCES quote_themes(id) ON DELETE CASCADE,
      user_id uuid REFERENCES profiles(id) ON DELETE CASCADE,
